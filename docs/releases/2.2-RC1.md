...
RCC6 does not expose a qualified USB-present signal to this firmware. It therefore does not pretend that voltage alone proves USB removal. Runtime projection starts after at least ten minutes of sustained falling-voltage samples. Battery size is descriptive metadata; ADC calibration remains the separate multimeter-derived `np battery offset <mV>` setting.
